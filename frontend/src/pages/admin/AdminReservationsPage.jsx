import { useEffect, useState } from "react";
import toast from "react-hot-toast";
import { reservationApi } from "../../services/api";
import LoadingSpinner from "../../components/LoadingSpinner";
import ReservationStatusBadge from "../../components/admin/ReservationStatusBadge";

export default function AdminReservationsPage() {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = () => reservationApi.listAll().then((res) => setItems(res.data.data ?? []));

  useEffect(() => {
    load().finally(() => setLoading(false));
  }, []);

  const updateStatus = async (id, status) => {
    await reservationApi.updateStatus(id, status);
    toast.success("Updated");
    load();
  };

  if (loading) return <LoadingSpinner />;

  return (
    <div className="dash-page-stack">
    <div className="dash-panel">
      <div className="dash-panel-header">
        <h2 className="dash-panel-title">All reservations</h2>
        <p className="text-sm text-slate-500">{items.length} requests</p>
      </div>
      {items.length === 0 ? (
        <p className="px-5 py-12 text-center text-slate-500 text-sm">No reservations yet.</p>
      ) : (
        <ul>
          {items.map((r) => (
            <li key={r.id} className="dash-row-card flex-wrap">
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-slate-900">{r.property?.title}</p>
                <p className="text-sm text-slate-500">{r.user?.email}</p>
                <p className="text-xs text-slate-400 mt-1">
                  {new Date(r.startDate).toLocaleDateString()} – {new Date(r.endDate).toLocaleDateString()}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                <ReservationStatusBadge status={r.status} />
                {r.status === "PENDING" && (
                  <>
                    <button
                      type="button"
                      onClick={() => updateStatus(r.id, "CONFIRMED")}
                      className="link-brand text-sm"
                    >
                      Confirm
                    </button>
                    <button
                      type="button"
                      onClick={() => updateStatus(r.id, "CANCELLED")}
                      className="text-sm font-semibold text-red-600 hover:underline"
                    >
                      Cancel
                    </button>
                  </>
                )}
              </div>
            </li>
          ))}
        </ul>
      )}
    </div>
    </div>
  );
}
